    ...
    @objc private func updateFavoriteList() {
        ...
        for containerView in stackView.subviews {
            if let button = containerView.subviews.first as? ServerButton {
                button.setImage(Theme.current.palette.appearance == .light ? Asset.Images.Piax.Tiles.quickConnectPlaceholderLight.image : Asset.Images.Piax.Tiles.quickConnectPlaceholderDark.image, for: .normal)
