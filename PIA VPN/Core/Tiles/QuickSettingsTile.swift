    ...
    @objc private func updateButtons() {
        ...
        if Flags.shared.enablesThemeSwitch {
            ...
            if AppPreferences.shared.currentThemeCode == ThemeCode.light {
                themeButton.setImage(Theme.current.palette.appearance == .light ? Asset.Images.Piax.Global.themeLightActive.image : Asset.Images.Piax.Global.themeDarkActive.image, for: [])
            } else {
                themeButton.setImage(Theme.current.palette.appearance == .light ? Asset.Images.Piax.Global.themeLightInactive.image : Asset.Images.Piax.Global.themeDarkInactive.image, for: [])
            }
        }
        ...
        if Client.preferences.isPersistentConnection {
            ...
            killSwitchButton.setImage(Theme.current.palette.appearance == .light ? Asset.Images.Piax.Global.killswitchLightInactive.image : Asset.Images.Piax.Global.killswitchDarkInactive.image, for: [])
        }

        if Client.preferences.nmtRulesEnabled {
            nmtButton.accessibilityLabel = L10n.Global.disable + " " + L10n.Tiles.Quicksetting.Nmt.title
            nmtButton.setImage(Theme.current.palette.appearance == .light ? Asset.Images.Piax.Global.nmtLightActive.image : Asset.Images.Piax.Global.nmtDarkActive.image, for: [])
        } else {
            nmtButton.accessibilityLabel = L10n.Global.enable + " " + L10n.Tiles.Quicksetting.Nmt.title
            nmtButton.setImage(Theme.current.palette.appearance == .light ? Asset.Images.Piax.Global.nmtLightInactive.image : Asset.Images.Piax.Global.nmtDarkInactive.image, for: [])
        }

        browserButton.setImage(Theme.current.palette.appearance == .light ? Asset.Images.Piax.Global.browserLightInactive.image : Asset.Images.Piax.Global.browserDarkInactive.image, for: [])
