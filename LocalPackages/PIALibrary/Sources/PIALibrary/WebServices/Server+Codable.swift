...
extension Server: Codable {
    ...
    private enum CodingKeys: String, CodingKey {
        ...
        // Only `dipToken` comes from the API; `DedicatedIPServerMapper` fills in the rest, which
        // must survive the `cachedServers` round trip — `dipUsername` keys the keychain password.
        case dipToken
        case dipUsername
        case dipExpire
        case dipStatus
