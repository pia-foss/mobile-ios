...
/// Covers what it takes for a Dedicated IP region to survive a relaunch: its credentials through
/// the cache's JSON round trip, and the user's selection through a preferences commit taken while
/// the server list does not carry the region.
final class DedicatedIPPersistenceTests: XCTestCase {
    ...
    func testDedicatedIPCredentialsSurviveTheCacheRoundTrip() throws {
        ...
        // `dipUsername` keys the password in the keychain and is randomly generated, so it cannot
        // be recomputed once dropped.
    ...
    func testPreferredServerSurvivesACommitTakenWhileTheServerIsMissing() {
        ...
        // A refresh that does not carry the DIP region: the getter can no longer resolve it.
    ...
    func testDownloadKeepsCachedDedicatedIPServersWhenNoTokensAreReadable() throws {
        ...
        // GIVEN the keychain reports no DIP tokens — which also happens when it is simply
        // unreadable, since it is stored `kSecAttrAccessibleAfterFirstUnlock`
        ...
        // THEN the refresh does not drop the cached DIP server
