...
/// Describes all possible errors thrown within `ICMPPing`
...
public class ICMPPing: NSObject {
    ...
    private let _serial = DispatchQueue(label: "ICMPPing internal")
    ...
    private func socket(socket: CFSocket, didReadData data: Data?) {
        ...
        CFSocketInvalidate(self.socket);  //closes the socket, unless you set the option to not close on invalidation
